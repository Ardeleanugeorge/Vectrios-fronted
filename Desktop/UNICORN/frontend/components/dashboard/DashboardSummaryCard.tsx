...
export default function DashboardSummaryCard({ companyName, monthlyLoss, riiScore, benchmarkPct, riskLevel }: DashboardSummaryCardProps) {
  ...
  return (
    <div className={`mb-6 p-5 rounded-xl border ${riskBg} flex items-center justify-between gap-6 flex-wrap`}>
      <div>
        <p className="text-xs font-semibold uppercase tracking-widest text-gray-500 mb-1">{riskLevel || "Revenue Risk"}</p>
        ...
        <p className="text-sm text-gray-500 mt-1">Fix the playbook below to recover revenue.</p>
      </div>
      <div className="flex gap-4 flex-wrap">
        {riiScore && (
          <div className="text-center bg-white rounded-lg px-4 py-3 border border-gray-200 min-w-[80px]">
            <p className="text-xs text-gray-500 mb-1">RII Score</p>
        ...
        {benchmarkPct && (
          <div className="text-center bg-white rounded-lg px-4 py-3 border border-gray-200 min-w-[80px]">
            <p className="text-xs text-gray-500 mb-1">Benchmark</p>
